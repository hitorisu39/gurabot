...
@Subcommand({
    ...
    group: "player",
    name: "loss",
    description: "Shows a player's recently lost national #1s.",
})
export class SnipePlayerLossSubcommand extends AbstractOsuCommand {
    ...
    @Option("days", "How many days of recent losses to show.")
    @IsInteger(1, 90)
    ...
    public async execute(ctx: CommandContext): Promise<void> {
        ...
        const result = await this.snipeService.playerChanges(profile.id, ESnipePlayerChangeType.Loss, since, until);

        if (!result.changes.length) {
            throw new Exception(
                EApplicationError.NOT_FOUND,
                `${profile.username} has not lost any national #1s in the last ${days} days.`,
            );
        }
        ...
        const data: SnipePlayerChangesViewDto = {
            ...
            type: ESnipePlayerChangeType.Loss,
            profile,
            days,
            page: 1,
