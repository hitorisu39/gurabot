...
interface IIndexedHistoryPoint {
    index: number;
    point: IHistoryPoint;
}

interface IHistoryBreak {
    startIndex: number;
    endIndex: number;
    length: number;
}
...
interface IHistoryStats {
    highest: IIndexedHistoryPoint;
    lowestNonZero: IIndexedHistoryPoint | null;
    longestBreak: IHistoryBreak | null;
}

export class GraphHistoryService extends AbstractService {
    @Import() declare private readonly graphRendererService: GraphRendererService;

    @Trace()
    public async generate(rawPlaycounts: ReadonlyArray<IMonthlyPlaycount>): Promise<Buffer> {
        const points = this.normalizePlaycounts(rawPlaycounts);
        ...
        const labels = points.map((point) => DateFormatter.monthYear(point.date));
        const data = points.map((point) => point.count);

        const historyPlugin = this.createHistoryPlugin(points, stats);
    ...
    private normalizePlaycounts(rawPlaycounts: ReadonlyArray<IMonthlyPlaycount>): Array<IHistoryPoint> {
        const countsByMonth = new Map<number, number>();

        for (const entry of rawPlaycounts) {
        ...
        const result: Array<IHistoryPoint> = [];
    ...
    private calculateStats(points: ReadonlyArray<IHistoryPoint>): IHistoryStats {
        ...
        if (!firstPoint) {
            throw new Exception(
                ...
                "Cannot calculate history statistics from an empty dataset.",
            );
        }

        let highest: IIndexedHistoryPoint = {
            index: 0,
            point: firstPoint,
        };

        let lowestNonZero: IIndexedHistoryPoint | null =
            firstPoint.count > 0
                ? {
                      index: 0,
                      point: firstPoint,
    ...
    private findLongestBreak(points: ReadonlyArray<IHistoryPoint>): IHistoryBreak | null {
        let currentStart: number | null = null;
        let longest: IHistoryBreak | null = null;
    ...
    private createHistoryPlugin(points: ReadonlyArray<IHistoryPoint>, stats: IHistoryStats): Plugin<"line"> {
        return {
            id: "osu_graph_history",
            ...
            afterDatasetsDraw: (chart) => {
                ...
                if (stats.lowestNonZero) {
                    this.drawPointMarker(chart, stats.lowestNonZero, graphColors.secondary);
                }
            },
    ...
    private drawBreakRegion(
        chart: Chart<"line">,
        points: ReadonlyArray<IHistoryPoint>,
        longestBreak: IHistoryBreak,
    ): void {
    ...
    private drawPointMarker(chart: Chart<"line">, entry: IIndexedHistoryPoint, color: string): void {
        const xScale = chart.scales.x;
    ...
    private drawHeader(chart: Chart<"line">, stats: IHistoryStats): void {
        ...
        /*
         * Break icon is a little wider than the circles.
         */
