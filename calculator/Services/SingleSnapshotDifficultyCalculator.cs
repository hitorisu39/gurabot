...
internal static class SingleSnapshotDifficultyCalculator
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        CalculatorAccessors calculatorAccessors = accessors.GetOrAdd(
            calculator.GetType(),
            CalculatorAccessors.Create);

        calculatorAccessors.PreProcess(
            calculator,
            mods,
            cancellationToken);
        ...
        CancellationToken cancellationToken)
    {
        ...
        Skill[] skills = calculatorAccessors.CreateSkills(
            calculator,
            playableBeatmap,
            playableMods);

        DifficultyHitObject[] difficultyObjects = calculatorAccessors
            .SortObjects(
                calculator,
                calculatorAccessors.CreateDifficultyHitObjects(
                    calculator,
                    playableBeatmap,
                    playableMods))
            .ToArray();

        (IBeatmap progressiveBeatmap, List<HitObject> progressiveHitObjects) =
            ProgressiveBeatmapFactory.Create(
                playableBeatmap,
                topLevelObjectCount);

        for (int index = 0; index < topLevelObjectCount; index++)
        {
            progressiveHitObjects.Add(
                playableBeatmap.HitObjects[index]);
        }

        double snapshotEndTime = playableBeatmap
            .HitObjects[topLevelObjectCount - 1]
            .GetEndTime();
        ...
        return calculatorAccessors.CreateDifficultyAttributes(
            calculator,
            progressiveBeatmap,
            playableMods,
            skills);
    ...
    internal readonly record struct PreparedDifficultyContext(
        IBeatmap PlayableBeatmap,
        Mod[] PlayableMods);
    ...
    private sealed class CalculatorAccessors
    {
        ...
        public Skill[] CreateSkills(
            DifficultyCalculator calculator,
            IBeatmap beatmap,
            Mod[] mods)
        {
            return InvokeRequired<Skill[]>(
                createSkills,
                calculator,
                beatmap,
                mods);
        ...
        public DifficultyAttributes CreateDifficultyAttributes(
            ...
            Skill[] skills)
        {
            return InvokeRequired<DifficultyAttributes>(
                createDifficultyAttributes,
                calculator,
                beatmap,
                mods,
    ...
    private static T InvokeRequired<T>(
        MethodInfo method,
        object target,
    ...
    private static class ProgressiveBeatmapFactory
    {
        private static readonly Type progressiveBeatmapType =
            typeof(DifficultyCalculator).GetNestedType(
                "ProgressiveCalculationBeatmap",
                BindingFlags.NonPublic)
            ?? throw new TypeLoadException(
                "DifficultyCalculator.ProgressiveCalculationBeatmap was not found. " +
                "The pinned ppy.osu.Game API may have changed.");
        ...
        private static readonly ConstructorInfo constructor =
            progressiveBeatmapType.GetConstructor(
                ...
                modifiers: null)
            ?? throw new MissingMethodException(
                progressiveBeatmapType.FullName,
                ".ctor(IBeatmap)");
        ...
        public static (IBeatmap Beatmap, List<HitObject> HitObjects) Create(
            IBeatmap baseBeatmap,
            int capacity)
        {
            object instance;
            ...
            {
                throw new InvalidOperationException(
                    $"{progressiveBeatmapType.FullName} does not implement " +
                    $"{typeof(IBeatmap).FullName}.");
            }

            if (hitObjectsField.GetValue(instance) is not List<HitObject> hitObjects)
            {
                throw new InvalidOperationException(
                    $"{progressiveBeatmapType.FullName}.HitObjects is not a " +
                    $"{typeof(List<HitObject>).FullName}.");
            }
